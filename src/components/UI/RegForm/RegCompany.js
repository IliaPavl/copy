...
const RegCompany = () => {
    ...
    async function setClient(e) {
        ...
        ClientHttpServise.setNewClient(companyName, nameCliet, phoneNumber, enabledStatus).then((respons) => {
            console.log(respons)
            alert(respons.data);
            //window.location.assign(URL_ALL_CLIENTS)
                ...
                <Form onSubmit={event => setClient(event)}>
                    ...
                    <Row className="mb-3">
                        <Form.Group as={Col} className="mb-3" controlId="exampleForm.ControlInput1">
                            ...
                            <Form.Control type="text" placeholder="+3752912345678" value={phoneNumber} onChange={event => setPhoneNumber(event.target.value)} />
                        </Form.Group>
